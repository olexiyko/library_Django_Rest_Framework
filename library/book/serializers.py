...
class BookSerializator(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if author_ids:
            ...
            print(existing_authors.values_list("id", flat=True))

            new_author_ids = set(author_ids) - set(existing_author_ids)
            for author_id in new_author_ids:
                Author.objects.create(
                    id=author_id, name="Default Name", surname="Default Surname"
                )
    ...
    def update(self, instance, validated_data):
        ...
        if author_ids:
            ...
            for author_id in new_author_ids:
                Author.objects.create(
                    pk=author_id, name="Default Name", surname="Default Surname"
                )
            authors = Author.objects.filter(id__in=author_ids)
